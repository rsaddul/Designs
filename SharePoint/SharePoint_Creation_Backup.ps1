...
# 8: Define site and specify which security groups will be applied to each library with edit permissions. (Used in Section 13 and References Variable 3)
$sitesLibrariesSecurityGroupsMapping = @{
    "$baseSharePointURL/$($siteCode)-$($staffSite)" = @{
        "General" = @("$allStaff")
        "Media" = @("$allStaff")
        # Add more sites, libraries and security groups as needed
    }
    ...
    # --------- DO NOT ADD THE HUB SITE ------ Add mappings for other sites as needed
}
...
$dialogResult = [System.Windows.Forms.MessageBox]::Show("Did you run the SharePoint Group script to create groups?", "SharePoint Groups", "YesNo", "Question")
if ($dialogResult -eq "Yes") {
    ...
    Write-Host "User confirmed security groups have not been created. Exiting the script." -ForegroundColor Red
    return
}
...
foreach ($group in @($allStaff, $allStudents, $allUsers)) { # Add more security groups as needed for additional checks
...
# Define the Hub Visitors group ID and Visitors group ID
$hubVisitorsGroupIds = @(12, 13)
$visitorGroupId = 4  
...
foreach ($site in $groupsForPermissionsAndAssociation) {
    ...
    # Check if the "default Hub Visitors" group exists on the site and remove if found
    foreach ($hubVisitorsGroupId in $hubVisitorsGroupIds) {
        $hubVisitorsGroup = Get-PnPGroup | Where-Object { $_.Id -eq $hubVisitorsGroupId }
        if ($hubVisitorsGroup) {
            Remove-PnPGroup -Identity $hubVisitorsGroupId -Force
            Write-Host "Removed group with ID '$hubVisitorsGroupId' from $siteUrl" -ForegroundColor Yellow
        }
    }
...
# Iterate over each site in the mappings
foreach ($siteUrl in $sitesLibrariesSecurityGroupsMapping.Keys) {
    ...
    foreach ($library in $sitesLibrariesSecurityGroupsMapping[$siteUrl].Keys) {
        Write-Host "Disabling inhertiance, removing default visitor group or hub visitor group and adding permissions to: $library" -ForegroundColor Green

        $groupsToAdd = $sitesLibrariesSecurityGroupsMapping[$siteUrl][$library]

        # Break role inheritance on the library
        $libraryObject = Get-PnPList -Identity $library
        $libraryObject.BreakRoleInheritance($true, $false) # BreakRoleInheritance method takes two boolean parameters

        # Remove group named "Hub Visitors" if it exists within the library
        $hubVisitorsGroup = Get-PnPGroup | Where-Object { $_.Title -eq "Hub Visitors" }
        if ($hubVisitorsGroup) {
            try {
                $libraryObject.RoleAssignments.GetByPrincipal($hubVisitorsGroup).DeleteObject()
                Invoke-PnPQuery
                Write-Host "The 'Hub Visitors' SharePoint group has been removed from the library '$library' on $($siteUrl)" -ForegroundColor Green
            } catch {
                Write-Host "An error occurred while removing the 'Hub Visitors' SharePoint group from the library '$library' in $($siteUrl)." -ForegroundColor Red
                Write-Host "Error details: $_" -ForegroundColor Red
            }
        } else {
            Write-Host "'Hub Visitors' SharePoint group not found in $($siteUrl). No action needed." -ForegroundColor Cyan
        }

        # Remove default visitors group using group ID 4 if it exists within the library
        try {
        ...
        # Add security groups to the library with edit permission level
        foreach ($group in $groupsToAdd) {
            Write-Host "Adding security group $group to library $library on site $siteUrl with edit permission level" -ForegroundColor Yellow
            Write-Host
            Set-PnPListPermission -Identity $library -User $group -AddRole "Edit"
        }
    }
    Disconnect-PnPOnline
}
